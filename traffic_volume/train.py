...
def create_lstm_model(input_shape=(24, 17), output_dim=16):
    """model 0 (Baseline): Bidirect LSTM"""
    ...
    return Model(inputs=input_layer, outputs=output_layer, name="Simple_LSTM_Model")

def create_simple_attention_model(input_shape=(24, 17), output_dim=16):
    """model 1 (Ablation Study): Bidirect LSTM + Attention (Deep DNN X)"""
    input_layer = Input(shape=input_shape, name="Input_Tensor")
    x = Bidirectional(LSTM(1000, return_sequences=True, activation='tanh'))(input_layer)
    x = Bidirectional(LSTM(500, return_sequences=True, activation='tanh'))(x)
...
def create_deep_attention_model(input_shape=(24, 17), output_dim=16):
    """model 2(Proposed model) Bidirect LSTM + Attention + Deep DNN"""
    input_layer = Input(shape=input_shape, name="Input_Tensor")
    x = Bidirectional(LSTM(1000, return_sequences=True, activation='tanh'))(input_layer)
    x = Bidirectional(LSTM(500, return_sequences=True, activation='tanh'))(x)
    x = Bidirectional(LSTM(100, return_sequences=True, activation='tanh'))(x)
...
def main():
    ...
    parser.add_argument('--model_type', default='simple_lstm', type=str, choices=['simple_lstm', 'simple_attention', 'deep_attention'], help='Model architecture to train.')
    ...
    if args.model_type == 'simple_lstm':
